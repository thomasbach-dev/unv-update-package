...
PARSER = argparse.ArgumentParser(
    ...
    epilog='\n'.join([
        ...
        f'{key}: {",".join(machines)}'
        for key, machines in MACHINE_SETS.items()
    ]),
...
class Configuration(typing.NamedTuple):
    ...
    def from_args(cls, args: argparse.Namespace):
        ...
        if args.machines is None:
            ...
            sys.exit(1)
        machines = [
            args.prefix + machine for machine in MACHINE_SETS.get(
                args.machines, args.machines.split(','))
        ]
        if args.docker_image is None:
            logger.error(
                'No Docker image provided. See the "--docker-image" option.')
            sys.exit(1)
        return cls(
...
def build_packages_and_get_paths(
        cfg: Configuration) -> typing.Iterable[pathlib.Path]:
...
def build_packages(cfg: Configuration,
                   directory: pathlib.Path) -> subprocess.CompletedProcess:
...
def get_package_from_stdout(
        proc: subprocess.CompletedProcess) -> typing.Iterable[str]:
...
def update_packages(cfg: Configuration,
                    packages: typing.Iterable[pathlib.Path]) -> None:
...
def copy_and_install_package(machine: str, pkg_path: pathlib.Path) -> None:
    ...
    subprocess.run(['scp', str(pkg_path), f'{machine}:{pkg_path.name}'],
                   check=True)
    logger.debug('Running dpkg-install')
    subprocess.run(['ssh', machine, f'dpkg --install {pkg_path.name}'],
                   check=True)


def is_package_installed(machine: str, package: str) -> bool:
    result = subprocess.run(
        ['ssh', machine, f'dpkg-query --status {package}'],
        check=False,
